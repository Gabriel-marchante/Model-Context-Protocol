...
export class McpConfigurationExtractor {
     ...
     */
    extractMcpConfigSectionByHeadings(readmeContent) {
        ...
        const mcpHeadings = headings.filter(heading => this.mcpKeywords.some(keyword => heading.toLowerCase().includes(keyword.toLowerCase())));
     ...
     */
    extractJsonConfigParagraphs(readmeContent) {
     ...
     */
    containsMcpConfiguration(content) {
     ...
     */
    findMcpKeywords(content) {
        const lowerContent = content.toLowerCase();
        return this.mcpKeywords.filter(keyword => lowerContent.includes(keyword.toLowerCase()));
    }
    /**
     ...
     */
    findParagraphStart(content, index) {
        let start = index;
     ...
     */
    findParagraphEnd(content, index) {
        let end = index;
        let newlineCount = 0;
