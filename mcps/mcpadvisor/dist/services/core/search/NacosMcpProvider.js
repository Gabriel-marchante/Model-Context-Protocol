...
export class NacosMcpProvider {
     ...
     * @returns Array of MCP server responses
     */
    async searchNacosMcpServers(query, keywords) {
        if (!this.mcpManager || !this.vectorDB) {
        ...
        try {
            ...
            const formattedVectorResults = (vectorResults || []).map((item) => {
     ...
     * @returns Array of MCP server responses
     */
    async keywordFallbackSearch(keywords) {
        if (!this.nacosClient) {
    ...
    async close() {
        ...
        const closePromises = [];
        if (this.mcpManager) {
            closePromises.push(this.mcpManager.stopSync().catch(error => {
                logger.warn('Error stopping MCP manager sync:', error);
            }));
        }
        if (this.nacosClient) {
            closePromises.push(this.nacosClient.close().catch(error => {
                logger.warn('Error closing Nacos client:', error);
            }));
        }
        if (this.vectorDB && typeof this.vectorDB.close === 'function') {
            closePromises.push(this.vectorDB.close().catch((error) => {
