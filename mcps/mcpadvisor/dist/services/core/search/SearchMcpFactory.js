 ...
 */
export async function searchOffline(query, options = {}, fallbackDataPath, textMatchWeight = 0.7) {
 ...
 * @returns Array of MCP server responses
 */
export async function searchGetMcp(query, options = {}) {
    const searchParams = typeof query === 'string'
        ? { taskDescription: query }
        : query;
    try {
 ...
 * @returns Array of MCP server responses
 */
export async function searchCompass(query, options = {}) {
    const searchParams = typeof query === 'string'
        ? { taskDescription: query }
        : query;
    try {
 ...
 * @returns Array of MCP server responses
 */
export async function searchNacosMcp(query, options = {}, config) {
    const searchParams = typeof query === 'string'
        ? { taskDescription: query }
        : query;
    try {
        ...
        const results = await service.search(searchParams, options);
        const duration = Date.now() - startTime;
        logger.info(`Nacos MCP search completed in ${duration}ms with ${results.length} results`, 'NacosMcpSearch', {
            duration,
            resultCount: results.length,
        });
        return results;
...
export function getSearchFunction(providerName) {
