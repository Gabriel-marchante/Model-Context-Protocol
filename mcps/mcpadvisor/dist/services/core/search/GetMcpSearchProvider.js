...
import { GetMcpResourceFetcher, } from '../../common/api/getMcpResourceFetcher.js';
import { processAndIndexData, performVectorSearch, } from './VectorSearchManager.js';
...
export class GetMcpSearchProvider {
     ...
     * 构造函数
     */
    constructor(resourceFetcher, cache, vectorEngine) {
        this.resourceFetcher =
            resourceFetcher || new GetMcpResourceFetcher(GETMCP_API_URL);
        this.cache = cache || new MemoryCache(CACHE_TTL_MS);
    ...
    async ensureDataLoaded() {
        ...
        try {
            ...
            this.cache.set(data);
            // 处理并索引数据
            await processAndIndexData(data, this.vectorEngine, GetMcpResourceFetcher.createSearchableText);
        }
        catch (error) {
            // 使用增强的日志记录方式，传递完整错误对象
            const message = error instanceof Error ? error.message : String(error);
            logger.error(`Error loading GetMCP data: ${message}`, {
                ...
                data: {
                    provider: 'GetMcpSearchProvider',
                    errorType: error instanceof Error ? error.constructor.name : typeof error,
                    apiUrl: this.resourceFetcher instanceof GetMcpResourceFetcher
