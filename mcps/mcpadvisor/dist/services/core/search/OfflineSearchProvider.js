...
export class OfflineSearchProvider {
    vectorEngine;
    config;
    dataLoaded = false;
    /**
    ...
    checkQueryForKeywords(query) {
        ...
        // 检查精确匹配
        const hasExactKeyword = exactKeywords.some(keyword => queryLower.includes(keyword.toLowerCase()));
        if (hasExactKeyword) {
            return true;
        }
        // 检查相关语义
        const hasRelatedPhrase = relatedPhrases.some(phrase => queryLower.includes(phrase.toLowerCase()));
        // 如果包含相关语义词组，也认为是相关查询
    ...
    async textSearch(query) {
        try {
            ...
            // 检查是否包含小红书相关服务器
            const redNoteServers = allServers.filter(server => server.id === 'rednote-mcp' || server.id === 'mcp-hotnews-server');
            console.log(`[DEBUG] 文本搜索 - 找到 ${redNoteServers.length} 个小红书相关服务器:`, redNoteServers.map(s => ({ id: s.id, title: s.title })));
            // 将查询分解为关键词
            ...
            for (const result of results) {
                ...
                const categoryMatch = Array.isArray(result.categories)
                    ? Math.max(...result.categories.map((cat) => this.calculateTextSimilarity(query, cat)))
                    : 0;
                const tagMatch = Array.isArray(result.tags)
                    ? Math.max(...result.tags.map((tag) => this.calculateTextSimilarity(query, tag)))
                    : 0;
                // 综合分数 (标题权重高)
                let similarity = titleMatch * 0.5 +
                    descMatch * 0.3 +
            ...
            // 过滤相似度低于阈值的结果
            const filteredResults = results.filter(result => (result.similarity || 0) >= this.config.minSimilarity);
    ...
    isRedNoteRelated(result) {
        ...
        // 检查标题
        if (redNoteKeywords.some(keyword => result.title.toLowerCase().includes(keyword.toLowerCase()))) {
            return true;
        }
        // 检查描述
        if (redNoteKeywords.some(keyword => result.description.toLowerCase().includes(keyword.toLowerCase()))) {
            return true;
        }
        // 检查分类
        if (Array.isArray(result.categories) &&
            result.categories.some(category => redNoteKeywords.some(keyword => category.toLowerCase().includes(keyword.toLowerCase())))) {
            return true;
        }
        // 检查标签
        if (Array.isArray(result.tags) &&
            result.tags.some(tag => redNoteKeywords.some(keyword => tag.toLowerCase().includes(keyword.toLowerCase())))) {
            return true;
        }
        return false;
    }
        ...
        for (const result of textResults) {
            if (result.id) {
                ...
                if (existing) {
                    ...
                    existing.similarity = Math.max(existing.similarity || 0, textWeightedSimilarity);
                }
                else {
                    // 如果不存在，添加新结果
                    const textWeightedSimilarity = (result.similarity || 0) * (this.config.textMatchWeight || 0.7);
