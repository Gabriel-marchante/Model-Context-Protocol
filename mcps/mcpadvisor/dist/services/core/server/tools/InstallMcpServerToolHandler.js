...
export class InstallMcpServerToolHandler extends BaseToolHandler {
    installationGuideService;
    configurationGuideService;
    constructor() {
        super();
    ...
    async handleRequest(request) {
        try {
            ...
            const installationGuide = await this.installationGuideService.generateInstallationGuide(sourceUrl, mcpName);
            // Generate client-specific configuration guide
            const configGuide = this.configurationGuideService.generateConfigurationGuide(mcpName, mcpClient);
            ...
            logger.error(`Error in InstallMcpServerToolHandler: ${error instanceof Error ? error.message : String(error)}`);
