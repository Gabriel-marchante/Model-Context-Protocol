...
(function (UpdateType) {
    UpdateType["VECTOR_DATA"] = "vector_data";
...
export class DataUpdateManager {
     ...
     * 初始化更新信息表
     */
    static async initUpdateInfoTable() {
        try {
            const sql = `
     ...
     */
    static async getLastUpdateTime(updateType) {
        try {
            const sql = `
     ...
     * @param updateType 更新类型
     */
    static async updateLastUpdateTime(updateType) {
        try {
            const now = new Date();
            const client = await getClient();
            // 检查记录是否存在
     ...
     * @returns 是否需要更新
     */
    static async needsUpdate(updateType, maxAgeHours = 1) {
