 ...
 * 构建插入值字符串
 */
const buildInsertValues = (values) => values
    .map(items => `(${items
    .map(item => typeof item.value === 'number'
    ? item.value
 ...
 */
const searchByMetadata = async (query, limit = 10, categories) => {
    try {
        if (!query.trim()) {
            return [];
        }
    ...
    `;
        ...
        const keywordConditions = keywords
            .map(keyword => `(server_name LIKE '%${keyword}%' OR description LIKE '%${keyword}%')`)
            .join(' OR ');
 ...
 * 使用内积计算归一化向量之间的相似度
 */
const searchVectorsByInnerProduct = async (vector, limit = 10, categories, minSimilarity = 0.5) => {
    try {
        // 动态调整 ef_search 参数，基于查询复杂度
        // 向量维度越高，ef_search 参数应越大
        const vectorDimension = vector.length;
        const efSearch = Math.max(global.systemEnv?.hnswEfSearch || 100, Math.min(200, Math.ceil(vectorDimension / 10)));
        // 构建基本查询
        let query = `
      BEGIN;
 ...
 */
const mergeSearchResults = (vectorResults, metadataResults, limit = 10) => {
    // 创建结果映射，以server_id为键
...
const initDatabaseSchema = async () => {
    ...
    `);
      ...
      `;
            const [checkResult] = await executeQuery(checkIndexSql);
...
export class OceanBaseClient {
     ...
     * @returns 搜索结果
     */
    async searchVectors(vector, limit = 10, options = {}) {
        if (!this.isInitialized) {
            await this.connect();
        }
        try {
            ...
            const [vectorResults, metadataResults] = await Promise.all([
                // 向量搜索
                searchVectorsByInnerProduct(normalizedQueryVector, limit, categories, minSimilarity),
                // 元数据搜索（如果有文本查询）
                textQuery
