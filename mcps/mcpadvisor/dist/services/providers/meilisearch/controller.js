...
import { MEILISEARCH_CONFIG, MeilisearchConfigManager } from '../../../config/meilisearch.js';
import { LocalMeilisearchController } from './localController.js';
import logger from '../../../utils/logger.js';
/**
 * 创建云端 Meilisearch 客户端
 */
const createCloudMeilisearchClient = (config) => {
    try {
...
export class MeilisearchClientFactory {
    static configManager = MeilisearchConfigManager.getInstance();
    /**
     * 创建主要客户端
     */
    static createPrimaryClient() {
 ...
 */
export class FailoverMeilisearchClient {
    primaryClient;
    fallbackClient;
    constructor() {
...
const createMeilisearchClient = () => {
    try {
        ...
        logger.info(`Legacy Meilisearch client initialized with host: ${MEILISEARCH_CONFIG.host}`);
        ...
        return {
            ...
            search: async (query, options = {}) => {
                try {
                    const results = await index.search(query, options);
                    logger.debug(`Legacy Meilisearch search for "${query}" returned ${results.hits.length} results`);
                    return results;
                }
                catch (error) {
                    const message = error instanceof Error ? error.message : String(error);
