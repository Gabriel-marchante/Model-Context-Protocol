 ...
 * 计算条目相似度
 */
const calculateEntrySimilarity = (entry, queryVector) => {
 ...
 * 按相似度排序结果
 */
const sortByHighestSimilarity = (a, b) => b.similarity - a.similarity;
/**
 * 内存向量搜索引擎实现
 * 用于开发和测试，或作为 OceanBase 的备选方案
 * 实现 IWritableVectorSearchEngine 接口，支持读写操作
 */
export class InMemoryVectorEngine {
    entries = [];
    /**
     * 构造函数
     */
        ...
        try {
            ...
            if (categories && categories.length > 0) {
                filteredEntries = this.entries.filter(entry => {
                    if (!entry.categories)
                        return false;
                    return categories.some(category => entry.categories?.some(entryCategory => entryCategory.toLowerCase().includes(category.toLowerCase())));
                });
            ...
            // 过滤相似度低于阈值的结果
            const filteredResults = results.filter(result => (result.similarity || 0) >= minSimilarity);
            // 按相似度降序排序并限制数量
