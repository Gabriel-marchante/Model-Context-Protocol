 ...
 * 自动加载兜底数据，支持混合搜索和过滤
 */
export class EnhancedMemoryVectorEngine {
    memoryEngine;
    offlineLoader;
    fallbackDataLoaded = false;
    lastFallbackLoadTime = 0;
    fallbackLoadIntervalMs = 3600000; // 1小时
    customFallbackPath;
    /**
     * 构造函数
     * @param customFallbackPath 可选的自定义兜底数据路径
     */
    constructor(customFallbackPath) {
     ...
     * 向量相似度搜索，支持混合搜索和过滤
     */
    async search(queryVector, limit = 10, options = {}) {
        try {
            // 确保兜底数据已加载
     ...
     * 确保兜底数据已加载
     */
    async ensureFallbackDataLoaded() {
        ...
        if (!this.fallbackDataLoaded ||
            ...
            try {
                ...
                // 检查是否包含小红书相关服务器
                const redNoteEntries = fallbackEntries.filter(entry => entry.data.id === 'rednote-mcp' || entry.data.id === 'mcp-hotnews-server');
                console.log(`[DEBUG] 其中包含 ${redNoteEntries.length} 个小红书相关服务器:`, redNoteEntries.map(e => ({ id: e.data.id, title: e.data.title })));
                // 只有在首次加载时才添加兜底数据
                // 这样可以避免重复添加相同的数据
                if (!this.fallbackDataLoaded) {
    ...
    async textSearch(query, limit) {
        try {
            ...
            const matchedResults = allResults.map(server => {
                const searchText = `${server.title} ${server.description} ${Array.isArray(server.categories) ? server.categories.join(' ') : ''} ${Array.isArray(server.tags) ? server.tags.join(' ') : ''}`.toLowerCase();
     ...
     * 合并向量搜索和文本搜索结果
     */
    mergeResults(vectorResults, textResults, limit) {
        // 创建结果映射，避免重复
    ...
    applyFilters(results, options) {
        ...
        if (options.minSimilarity !== undefined) {
            filtered = filtered.filter(result => (result.similarity || 0) >= (options.minSimilarity || 0));
        }
        // 应用类别过滤
        if (options.categories && options.categories.length > 0) {
            filtered = filtered.filter(result => {
                ...
                return options.categories.some(category => Array.isArray(result.categories) &&
                    result.categories.some((c) => c.toLowerCase().includes(category.toLowerCase())));
            });
        }
        // 应用标签过滤
        if (options.tags && options.tags.length > 0) {
            filtered = filtered.filter(result => {
                ...
                return options.tags.some(tag => Array.isArray(result.tags) &&
                    result.tags.some((t) => t.toLowerCase().includes(tag.toLowerCase())));
            });
        }
        return filtered;
    }
