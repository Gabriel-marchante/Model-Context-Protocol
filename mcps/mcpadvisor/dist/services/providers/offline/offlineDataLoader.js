...
// 备用路径，用于在默认路径无法访问时尝试
const ALTERNATIVE_FALLBACK_DATA_PATH = getNodeModulesPath('@xiaohui-wang/mcpadvisor', 'data/mcp_server_list.json');
/**
 * 离线数据加载器类
 */
export class OfflineDataLoader {
    fallbackDataPath;
    disableFallbackPaths;
    /**
     * 构造函数
     * @param fallbackDataPath 可选的自定义兜底数据路径
     * @param options 选项对象，包含 disableFallbackPaths 属性
     */
    constructor(fallbackDataPath, options = {}) {
    ...
    async loadFallbackData() {
        try {
            ...
            if (!fs.existsSync(this.fallbackDataPath)) {
                ...
                // 尝试备用路径
                if (this.fallbackDataPath === DEFAULT_FALLBACK_DATA_PATH &&
            ...
            const redNoteServers = parsedData.filter((item) => item.id === 'rednote-mcp' || item.id === 'mcp-hotnews-server');
            logger.info(`Found ${redNoteServers.length} RedNote related servers in raw data:`, {
                redNoteServers: redNoteServers.map((s) => ({ id: s.id, name: s.name }))
            });
            // 转换为MCPServerResponse格式
            const serverResponses = parsedData.map((item) => ({
     ...
     * @returns 带有嵌入向量的兜底数据
     */
    async loadFallbackDataWithEmbeddings() {
        try {
            ...
            // 检查是否包含小红书相关服务器
            const redNoteServers = serverResponses.filter(server => server.id === 'rednote-mcp' || server.id === 'mcp-hotnews-server');
            logger.info(`原始数据中包含 ${redNoteServers.length} 个小红书相关服务器:`, redNoteServers.map(s => ({ id: s.id, title: s.title })));
            const result = [];
            // 如果没有服务器数据，直接返回空结果
            ...
            for (const server of serverResponses) {
                try {
                    // 生成文本用于嵌入
                    const textForEmbedding = `${server.title}. ${server.description}. ${Array.isArray(server.categories) ? server.categories.join(', ') : ''}. ${Array.isArray(server.tags) ? server.tags.join(', ') : ''}`;
                    // 获取嵌入向量
