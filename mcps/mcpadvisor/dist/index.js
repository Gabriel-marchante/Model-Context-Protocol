...
import { ServerService, TransportType } from './services/core/server/index.js';
    ...
    try {
        ...
        if (nacosServerAddr && nacosUsername && nacosPassword) {
            try {
                ...
                logger.error(`Failed to initialize Nacos MCP provider: ${error instanceof Error ? error.message : String(error)}`, { error });
            }
        }
        else {
            logger.warn('Nacos MCP provider not initialized: Missing required environment variables (NACOS_SERVER_ADDR, NACOS_USERNAME, NACOS_PASSWORD)');
        }
...
// Helpers
async function ensureLocalMeilisearch() {
