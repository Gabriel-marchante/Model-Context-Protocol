...
const consoleFormat = winston.format.combine(winston.format.timestamp({ format: 'YYYY-MM-DD HH:mm:ss.SSS' }), winston.format.colorize({ all: false }), winston.format.printf(info => {
...
const fileFormat = winston.format.combine(winston.format.timestamp({ format: 'YYYY-MM-DD HH:mm:ss.SSS' }), winston.format.uncolorize(), winston.format.printf(info => {
    // Add context information if available
    const context = info.context ? ` [${info.context}]` : '';
    // 处理错误对象和堆栈信息
...
transports.push(new winston.transports.Console({
    ...
    stderrLevels: ['error', 'warn', 'info', 'http', 'debug'], // Send all levels to stderr
}));
...
function createLogMethod(level) {
    return function (message, contextOrData, data) {
