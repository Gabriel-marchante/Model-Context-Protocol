 ...
 * 检查结果中是否包含指定的服务器名称
 */
export function containsServerNames(results, serverNames) {
    ...
    for (const serverName of serverNames) {
        const found = results.some(result => {
            ...
            return (serverInfo.id === serverName ||
 ...
 * 检查结果中是否包含指定关键词
 */
export function containsKeywords(results, keywords) {
 ...
 * 创建搜索测试函数
 */
export function createSearchTest(testCase, searchFn, fallbackPath) {
    const testFn = async () => {
        ...
        try {
            ...
            if (results.length > 0) {
                // 输出服务器列表，便于调试
                console.log(`[${testCase.name}] 返回的服务器:`, results.map((item) => ({
                    id: item.id,
                    name: item.name || '',
                ...
                if (expectedKeywords && expectedKeywords.length > 0) {
                    ...
                    if (matchedItems.length > 0) {
                        console.log(`[${testCase.name}] 匹配关键词的项目:`, matchedItems.map(item => {
                            const itemInfo = {
                                title: item.title,
                                tags: item.tags,
                            };
